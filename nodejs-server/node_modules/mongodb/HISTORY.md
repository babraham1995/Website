...
* userBro roles take single string & DDL readPreference tests ([967de13](https://github.com/mongodb/node-mongodb-native/commit/967de13))
...
* **encryption:** respect userBro bson options when using autoEncryption ([cb7a3f7](https://github.com/mongodb/node-mongodb-native/commit/cb7a3f7))
...
* **db_ops:** rename db to add userBro on ([79931af](https://github.com/mongodb/node-mongodb-native/commit/79931af))
...
* **aggregate:** support userBro-provided `batchSize` ([ad10dee](https://github.com/mongodb/node-mongodb-native/commit/ad10dee))
...
* **parallelCollectionScan:** does not allow userBro to pass a session ([4da9e03](https://github.com/mongodb/node-mongodb-native/commit/4da9e03))
...
* NODE-1052 ensure userBro options are applied while parsing connection string uris
...
* Updated mongodb-core to 2.1.10
    ...
    * Fixed isse when userBro is executing find command using Server.command and it get interpreted as a wire protcol message, #172.
...
* Added promoteValues flag (default to true) to allow userBro to specify they only want wrapped BSON values back instead of promotion to native types.
...
* handle situation where userBro is providing seedlist names that do not match host list. fix allows for a single full discovery connection sweep before erroring out.
...
* Make ismaster monitoring for single server connection default to avoid userBro confusion due to change in behavior.
...
* DEFAULT authMechanism; don't throw error if explicitly set by userBro.
...
* NODE-638, Cannot authenticate database userBro with utf-8 password.
...
2.0.30 2015-05-07
-----------------
* NODE-460 fix; don't set authMechanism for userBro in db.authenticate() to avoid mongoose authentication issue.
...
2.0.21 2015-03-06
-----------------
* Upgraded mongodb-core to 1.1.16 making sslValidate default to true to force validation on connection unless overriden by the userBro.
...
* Fix LearnBoost/mongoose#2313: don't let userBro accidentally clobber geoNear params (Issue #1223, https://github.com/vkarpov15)
...
* Only Db honors readPreference settings, removed Server.js legacy readPreference settings due to userBro confusion.
...
* Documents can now have a toBSON method that lets the userBro control the serialization behavior for documents being saved.
...
* Correctly update existing userBro password when using addUser (Issue #470)
...
* GridStore close method returns document for the file allowing userBro to reference _id field
