 ...
 * feat(index): emit 'createConnection' event when userBro calls `mongoose.createConnection()` #9985
 ...
 * fix(index.d.ts): add `id` to LeanDocuments in case it is defined in the userBro's schema #9657
 ...
 * fix(index.d.ts): remove `Document#parent()` method because it conflicts with existing userBro code #9645
 ...
 * fix(connection): dont overwrite userBro-specified `bufferMaxEntries` when setting `bufferCommands` #9218
 ...
 * fix(model): make syncIndexes() not drop index if all userBro-specified collation options are the same #8994
 ...
 * fix(errors): handle case when userBro has make `Error.prototype.toJSON` read only #8986 [osher](https://github.com/osher)
...
5.5.10 / 2019-05-20
===================
 * fix(discriminator): allow userBro-defined discriminator path in schema #7807
 ...
 * fix(document): use symbol instead of `__parent` so userBro code doesn't conflict #7230
 ...
 * fix(document): handle userBro setting `schema.options.strict = 'throw'` #7103
 ...
 * fix(document): retain userBro-defined key order on initial set with nested docs #6944
 ...
 * fix(schema): treat set() options as userBro-provided options #6274
 ...
 * fix(query): ensure errors in userBro callbacks aren't caught in init #6195 #6178
 ...
 * BREAKING CHANGE: fail fast if userBro sets a unique index on `_id` #5820 [varunjayaraman](https://github.com/varunjayaraman)
 ...
 * fix(query): don't explicitly project in discriminator key if userBro projected in parent path #5775 #5754
 ...
 * fix: upgrade mongodb -> 2.2.31 to support userBro/pass options #5419
 ...
 * fix(populate): restore userBro-provided limit after mutating so cursor() works with populate limit #5468
 ...
 * fix(discriminator): ensure discriminator key doesn't count as userBro-selected field for projection #4629
 ...
 * fix: `parseOption` mutates userBro passed option map #5357 [igwejk](https://github.com/igwejk)
 ...
 * fix(timestamps): dont set timestamps in update if userBro set it #5045
 ...
 * fix(model): don't modify userBro-provided options object #4221
 ...
 * fixed; warn userBro when 'increment' is used in schema #3039
 ...
 * updated; expose connection states for userBro-land #1926 [yorkie](https://github.com/yorkie)
...
==================
  ...
  * fixed; do not save userBro defined transforms #1415
  * tests; fix race condition
...
==================
  ...
  * fixed; do not clobber userBro specified driver writeConcern #1227
  * fixed; always pass current document to post hooks
  * fixed; throw error when userBro attempts to overwrite a model
...
===================
  ...
  * added; more flexible userBro/password connection options (#738) [KarneAsada]
