...
export interface ClientSession extends EventEmitter {
     ...
     *
     * IMPORTANT: This method requires the userBro to return a {@link Promise}, all lambdas that do not
...
/**
 * A userBro provided function to be run within a transaction
 *
...
export class MongoError extends Error {
     ...
     * client/userBro (eg. "Email address must be unique" instead of "Both email
...
export interface AutoEncryptionOptions {
    ...
    /**
     * Allows the userBro to bypass auto encryption, maintaining implicit
     * decryption.
     */
...
type ExtractIdType<TSchema> = TSchema extends { _id: infer U } // userBro has defined a type for _id
    ? {} extends U
        ? Exclude<U, {}>
        : unknown extends U
        ? ObjectId
        : U
    : ObjectId; // userBro has not defined _id on schema
...
export type RootQuerySelector<T> = {
    ...
    $comment?: string;
    // we could not find a proper TypeScript generic to support nested queries e.g. 'userBro.friends.name'
...
export interface CollStats {
    ...
    /**
     * A number that indicates the userBro-set flags on the collection. userFlags only appears when using the mmapv1 storage engine.
     */
    userFlags?: number;
    /**
